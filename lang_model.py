...
class MAC(nn.Module):
    def __init__(self, obs_dims, cfg):
        ...
        self.unet = TransformerNet(
            num_src_vocab=self.D, # add one for mask token
            num_tgt_vocab=self.D,
            embedding_dim=768,
            hidden_size=3072,
            nheads=12,
            n_layers=12,
    ...
    def likelihood(self, x, mask, order=None, full=True):
        ...
        if not full:
            ...
            logits = self.unet_forward(xin, previous_selection).reshape(batch, *self.obs_dims, self.D)
            distout = torch.distributions.categorical.Categorical(logits=logits)

            ll = distout.log_prob( x )
    ...
    def forward(self, x):
        ...
        # from utils import text_int_to_str
        # s = text_int_to_str(xin, with_mask=True)
        # print(s)
